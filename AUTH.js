...
/* ============================================================
   js/auth.js — depends on js/supabase-client.js (loaded first)
   Exposes window.Auth: signUp, signIn, signOut, getSession,
   getProfile, requestPasswordReset, updatePassword, onAuthChange
   ============================================================ */
...
async function signUp({ firstName, lastName, email, phone, city, password, role = 'student' }) {
  ...
  if (role === 'instructor') {
    return {
      ok: true, data,
      notice: 'Account created. Please verify your email, then wait for admin approval before logging in.',
    };
  }
  return { ok: true, data, notice: 'Account created! Please check your email to verify your account.' };
}
...
window.Auth = { signUp, signIn, signOut, getSession, getProfile, requestPasswordReset, updatePassword, onAuthChange };
